  ...
  async processClipWithProgress(
    ...
    return new Promise((resolve, reject) => {
      ...
      if (options.videoFade && options.fadeDuration) {
        const fadeIn = `fade=t=in:st=0:d=${options.fadeDuration}`;
        const fadeOut = `fade=t=out:st=${parseFloat(duration) - options.fadeDuration}:d=${options.fadeDuration}`;
        ffmpegCmd[ffmpegCmd.indexOf('-vf') + 1] += `,${fadeIn},${fadeOut}`;
      }
      ...
      // Quality settings
      const qualityMap = {
        high: ['-c:v', 'libx264', '-crf', '18', '-preset', 'fast'],
        balanced: ['-c:v', 'libx264', '-crf', '20', '-preset', 'medium'],
        compressed: ['-c:v', 'libx264', '-crf', '23', '-preset', 'fast'],
      };
